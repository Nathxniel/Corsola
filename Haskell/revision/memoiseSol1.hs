...
executeStatement (Assign id e) f p s
  = updateVar (id, eval e f s) s
executeStatement (AssignA id e1 e2) f p s
  = updateVar (id, newArray) s
  where
    newArray = assignArray (getValue id s) (eval e1 f s) (eval e2 f s)
executeStatement (If e b1 b2) f p s
  | eval e f s == I 1 = executeBlock b1 f p s
  | otherwise         = executeBlock b2 f p s
executeStatement statement @ (While e b) f p s
  | eval e f s ==  I 1 = executeStatement statement f p (executeBlock b f p s)
  | otherwise          = s
executeStatement (Call id func es) f p s
  | id == ""  = returnS
  | otherwise = updateVar (id, getValue "$res" returnS) (getGlobals(returnS))
  where
    (as, e) = lookUp func p
    globalS = getGlobals s
    vs      = evalArgs es f s
    bound   = bindArgs as vs
    newS    = executeBlock e f p (bound ++ globalS)
    returnS = getLocals s ++ newS
executeStatement (Return e) f p s
  = updateVar ("$res", eval e f s) (getGlobals s)
  
executeBlock :: Block -> [FunDef] -> [ProcDef] -> State -> State
-- Pre: All code blocks and associated statements are well formed
executeBlock [] _ _ s
  = s
executeBlock (s : ss) f p st
  = executeBlock ss f p (executeStatement s f p st)
