...
lookUp i ((t,v):tbl)
  | i == t    = v
  | otherwise = lookUp i tbl

tryToLookUp :: Eq a => a -> b -> [(a, b)] -> b
tryToLookUp i d []
  = d
tryToLookUp i d ((t,v):tbl)
  | i == t    = v
  | otherwise = tryToLookUp i d tbl
...
occurs str (TFun ta tb)
  = occurs str ta || occurs str tb
occurs str (TVar var)
  = str == var
occurs _ _
  = False
...
inferType (Prim prim) e
  = lookUp prim primTypes

inferType (Cond ta tb tc) e
  | infa /= TBool = TErr
  | infb /= infc  = TErr
  | otherwise     = infb
  where
...
inferType (App ta tb) e
  = inferApp (inferType ta e) (inferType tb e) e
  where
...
applySub _ TBool
  = TBool
applySub s (TFun ta tb)
  = TFun (applySub s ta) (applySub s tb)
applySub s (TVar str)
  = tryToLookUp str (TVar str) s
applySub _ _
  = TErr
...
unifyPairs [] acc
  = Just acc
unifyPairs ((TInt, TInt):ts) acc
  = unifyPairs ts acc
unifyPairs ((TBool, TBool):ts) acc
  = unifyPairs ts acc
unifyPairs ((TVar v, TVar v'):ts) acc
  | v == v'   = unifyPairs ts acc
  | otherwise = Nothing
