...
getAttribute x (Element _ as _)
  = fromMaybe "" $ lookup x as
getAttribute _ _
  = ""

getChildren :: String -> XML -> [XML]
getChildren x (Element _ _ cs)
  = concatMap find cs
  where
    find e@(Element n _ _)
      | n == x    = e : getChildren x e
      | otherwise = getChildren x e
    find _
      = []
getChildren _ _
  = []
...
addChild x (Element n as cs)
  = Element n as (cs ++ [x])

getValue :: XML -> XML
getValue (Element _ _ cs)
  = Text (concatMap glue cs)
  where
    glue Null
      = ""
    glue (Text t)
...
parseAttributes s 
  ...
  where
    primary u acc
      | x == '>'  = (acc, xs)
      | otherwise = primary (skipSpace rest) ((name, this):acc)
      where
        a@(x : xs)
          = skipSpace u
        (name, att)
        ...
        (r : rs)
          = skipSpace att
        (this, rest) 
          = span (flip notElem "\" ") (skipSpace rs)
