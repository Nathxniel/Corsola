...
assignArray (A as) (I i) (I v)
  = A ((i,v) : [a | a@(i',_) <- as, i' /= i])

updateVar :: (Id, Value) -> State -> State
updateVar (n, v) ss
  = ad $ map (\a@(n', (sc, v')) -> if n == n' then (n, (sc, v)) else a) ss
  where
    ad xs
      | xs == ss  = (n, (Local, v)) : xs
      | otherwise = xs
...
applyOp Mul (I n) (I m)
  = I (n * m)
applyOp Less (I n) (I m)
  = I (fromEnum $ n < m)
applyOp Equal (I n) (I m)
  = I (fromEnum $ n == m)
...
eval (Var x) _ s
  = getValue x s
eval (OpApp o n m) fs s
  = applyOp o (eval n fs s) (eval m fs s)
eval (Cond p t e) fs s
  | eval p fs s == I 1 = eval t fs s
  | otherwise          = eval e fs s
eval (FunApp f es) fs s
  = eval e fs (binds ++ s)
  where
    binds   = bindArgs as vs
    vs      = evalArgs es fs s
    (as, e) = lookUp f fs
...
executeStatement (Assign n e) fs _ ste
  = updateVar (n, eval e fs ste) ste 
executeStatement (AssignA n e1 e2) fs _ ste
  = updateVar (n, assignArray ...
executeStatement (If e b1 b2) fs ps ste 
  | eval e fs ste == I 1 = executeBlock b1 fs ps ste
  | otherwise            = executeBlock b2 fs ps ste
executeStatement (While e b) fs ps ste
  | eval e fs ste == I 1 = ting
  | otherwise            = executeStatement (While e b) fs ps (ting ++ ste)
  where
    ting = executeBlock b fs ps ste

executeBlock :: Block -> [FunDef] -> [ProcDef] -> State -> State
-- Pre: All code blocks and associated statements are well formed
executeBlock _ _ _ s
  = s
